...
from src.basegame_flappy import SCREENWIDTH, SCREENHEIGHT, PIPEGAPSIZE, BASEY, PLAYERS_LIST, BACKGROUNDS_LIST, \
    PIPES_LIST, getHitmask, pixelCollision
from src.forward_model import getNextPipe
...
def visualize(model, verbose=False):
    ...
    crashInfo = mainGame(movementInfo, model, verbose)
...
def mainGame(movementInfo, model, verbose):
    ...
    while True:
        nextPipeIndex = getNextPipe(playerx, upperPipes, IMAGES['pipe'][0].get_width())
