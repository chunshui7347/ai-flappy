...
def wid170047_score(game, player):
    """Calculate the heuristic value of a game state from the point of view
    of the given player.
    ...
    m = 1 - blank_spaces / total_spaces

    # early game strategy is to penalized position nearer to the boundary
    if m < 0.2:
        ...
        dist = (cw - x) ** 2 + (ch - y) ** 2
        return (my_moves - opponent_moves) - 0.6 * math.sqrt(dist)

    # mid game strategy is to be aggressive
    elif m < 0.8:
        # taken from function weighted_chances_heuristic
        my_moves = len(game.get_legal_moves(player))
        opponent_moves = len(game.get_legal_moves(game.get_opponent(player)))

        return my_moves*my_moves - 1.5*opponent_moves*opponent_moves

    # late game strategy is to be defensive
    else:
        # taken from function weighted_chances_heuristic_2
        my_moves = len(game.get_legal_moves(player))
        opponent_moves = len(game.get_legal_moves(game.get_opponent(player)))
